...
gh copilot --version &>/dev/null 2>&1 && HAS_COPILOT=true
...
- **Codex** (if \`has_codex: true\`):
  \`codex review --base main --title "\$(git log --format=%s -1)" > /tmp/create-pr-codex.md 2>&1\`

- **Copilot** (if \`has_copilot: true\`):
  \`gh copilot -p "Review the changes on this branch compared to main. Run 'git diff \$(git merge-base HEAD main)..HEAD' to see the diff. Focus on bugs, security issues, and improvements. Be specific about file paths and line numbers. Format each finding as: **[severity]** file:line -- description" --allow-all-tools > /tmp/create-pr-copilot.md 2>&1\`
...
- \`/tmp/create-pr-codex.md\` (if Codex was launched)
- \`/tmp/create-pr-copilot.md\` (if Copilot was launched)
...
Clean up output files: \`rm -f /tmp/create-pr-codex.md /tmp/create-pr-copilot.md\`
