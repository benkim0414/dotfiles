...
gh extension list 2>/dev/null | grep -q copilot && HAS_COPILOT=true
...
Clean up output files: \`rm -f /tmp/create-pr-\$(git branch --show-current)-codex.md /tmp/create-pr-\$(git branch --show-current)-copilot.md\`
