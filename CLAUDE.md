...
style had loaded. The style alone does not fix readability; the caveman
default must be `off`. `CAVEMAN_DEFAULT_MODE=off` in front of a command is
the quickest way to suppress it for one invocation.
